...
def test_register_creates_player_and_start(client, _test_engine):
    with ih.registered_player(client, _test_engine, "reg0") as body:
...
def test_login_with_valid_code_works(client, _test_engine):
    with ih.registered_player(client, _test_engine, "login") as body:
...
def test_world_state_has_home_influence_block(client, _test_engine):
    with ih.registered_player(client, _test_engine, "infls"):
...
def test_world_window_z_changes_and_is_deterministic(client, _test_engine):
    with ih.registered_player(client, _test_engine, "z"):
...
def test_health_and_ready_endpoints(client):
    ...
    h = health.get_json()
    assert h.get("status") == "ok" and h.get("app") == "GuardStar" and "build_id" in h
...
def test_world_state_endpoint_shows_tick_and_unit(client, _test_engine):
    with ih.registered_player(client, _test_engine, "state"):
...
def test_fleet_save_deducts_metal_when_adding_ships(client, _test_engine):
    with ih.registered_player(client, _test_engine, "fsave"):
...
def test_fleet_merge_then_split(client, _test_engine):
    with ih.registered_player(client, _test_engine, "merge"):
        me = client.get("/api/me").get_json()
        pid = me["planets"][0]["id"]
        a = client.post(
            "/api/fleets/create",
...
def test_fleet_disband_returns_deleted(client, _test_engine):
    with ih.registered_player(client, _test_engine, "disb"):
        me = client.get("/api/me").get_json()
        pid = me["planets"][0]["id"]
        cr = client.post(
            "/api/fleets/create",
