...
class OutpostService:
    ...
    def supply_route_logistics_costs(
        ...
        if self._balance and isinstance(
            getattr(getattr(self._balance, "pack", None), "economy", None), dict
        ):
    ...
    def apply_supply_route_logistics_tick(self, s: Session, *, tick: int) -> None:
        ...
        for op in outposts:
            if int(getattr(op, "z", 0) or 0) != 0:
                continue
            if self._cell_is_owned_planet_tile(
                s, owner_id=op.owner_player_id, x=int(op.x), y=int(op.y), z=0
            ):
                continue
            ...
            mul = 1.0
            if self._balance and getattr(
                getattr(self._balance, "pack", None), "races_by_id", None
            ):
                pr = s.execute(
                    select(Player.race_id).where(Player.id == op.owner_player_id)
                ).scalar_one_or_none()
                if pr:
                    rd = self._balance.pack.races_by_id.get(str(pr))
                    mods = (rd.get("modifiers") if isinstance(rd, dict) else None) or {}
                    if isinstance(
                        mods.get("supply_route_upkeep_multiplier"), (int, float)
                    ):
            ...
            if need_f <= 0 and need_w <= 0:
                continue
            res = s.execute(
                select(Resource).where(Resource.planet_id == hub.id)
            ).scalar_one_or_none()
            if not res:
                continue
            if (
                int(getattr(res, "food", 0) or 0) >= need_f
                and int(getattr(res, "water", 0) or 0) >= need_w
            ):
    ...
    def apply_outpost_upkeep_tick(self, s: Session, *, tick: int) -> None:
        ...
        outposts = (
            s.execute(select(Outpost).where(Outpost.status.in_(["active", "offline"])))
            .scalars()
            .all()
        )
        if not outposts:
