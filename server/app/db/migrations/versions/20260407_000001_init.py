...
def upgrade() -> None:
    ...
    op.create_index("ix_players_access_code_hash", "players", ["access_code_hash"], unique=True)
    ...
    op.create_index("ix_planets_owner_player_id", "planets", ["owner_player_id"], unique=False)
    op.create_index("ix_planets_pos_x", "planets", ["pos_x"], unique=False)
    op.create_index("ix_planets_pos_y", "planets", ["pos_y"], unique=False)

    op.create_table(
        "resources",
        sa.Column("planet_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("planets.id", ondelete="CASCADE"), primary_key=True, nullable=False),
    ...
    op.create_index("ix_units_owner_player_id", "units", ["owner_player_id"], unique=False)
