...
class Player(Base):
    ...
    research_points: Mapped[float] = mapped_column(Numeric(16, 6), nullable=False, default=0)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
    last_login_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    # Включить запись API-действий (POST/PUT/PATCH/DELETE) в feedback_playtest_api_logs
    feedback_audited: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, index=True)
