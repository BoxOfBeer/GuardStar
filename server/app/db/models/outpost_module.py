...
class OutpostModule(Base):
    ...
    __table_args__ = (UniqueConstraint("outpost_id", "slot_idx", name="uq_outpost_modules_slot"),)

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    outpost_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("outposts.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    finish_tick: Mapped[int] = mapped_column(Integer, nullable=False, default=0, index=True)
